...
#include <cmath>
// Main kernal for pthcoherence NDT
    ...
    int num_samples, int num_rx_elements, int nX, int nZ) // same format as that of CPU code
{
    ...
    if (Xi >= nX || Zi >= nZ)
        return;
    // Loading the pixel and the transmitter location
    ...
    for (int ex = 0; ex < num_rx_elements; ex++)
    {
        ...
        if (samples >= 0 && samples < num_samples)
        {
            ...
            das_sum += s_i;
            double val_p_root = pow(fabs(s_i), 1.0 / p); // pthroot
            p_root_sum += (s_i < 0.0) ? -val_p_root : val_p_root;
            sum_sq += s_i * s_i;
        }
    }
    // pth coherence factor computation
...
{
    // Standard error checks for inputs, outputs, datatypes, dimension mismatches and empty arrays
    if (nrhs != 9)
    {
    ...
    // Allocating memory on GPU for the arrays
    ...
    // Assigning the structure for threads and blocks
    ...
    // Ensuring all threads are synchronised and no errors are found
